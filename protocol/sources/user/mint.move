...
public fun mint<T>(
    ...
    let (balance_sheet_cash, balance_sheet_debt, balance_sheet_revenue, _) = reserve::balance_sheet(
        balance_sheet,
    );
    assert!(
        balance_sheet_cash + balance_sheet_debt - balance_sheet_revenue + deposit_amount <= supply_limit,
