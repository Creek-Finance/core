...
public struct Obligation has key, store {
    id: UID,
    balances: BalanceBag,
...
public struct ObligationOwnership has drop {}
...
public struct ObligationRewardsPointRedeemed has copy, drop {
    obligation: ID,
    witness: TypeName,
    amount: u64,
}

public struct ObligationLocked has copy, drop {
    obligation: ID,
    witness: TypeName,
    borrow_locked: bool,
    repay_locked: bool,
...
public struct ObligationUnlocked has copy, drop {
    obligation: ID,
    witness: TypeName,
}
...
public fun obligation_key_uid_mut_delegated(
    key: &mut ObligationKey,
    _: Witness<ObligationKey>,
): &mut UID {
    &mut key.id
}
...
public fun obligation_uid_mut_delegated(
    obligation: &mut Obligation,
    _: Witness<Obligation>,
): &mut UID {
    &mut obligation.id
}

public(package) fun new(ctx: &mut tx_context::TxContext): (Obligation, ObligationKey) {
...
public(package) fun accrue_interests_and_rewards(obligation: &mut Obligation, market: &Market) {
    let debt_types = debt_types(obligation);
    let mut i = 0;
    let n = vector::length(&debt_types);
    while (i < n) {
        let type_name = *vector::borrow(&debt_types, i);
        let new_borrow_index = market::borrow_index(market, type_name);
        obligation_debts::accrue_interest(&mut obligation.debts, type_name, new_borrow_index);

        i = i + 1;
    };
}

public(package) fun withdraw_collateral<T>(self: &mut Obligation, amount: u64): Balance<T> {
    let type_name = type_name::get<T>();
    obligation_collaterals::decrease(&mut self.collaterals, type_name, amount);
    balance_bag::split(&mut self.balances, amount)
}

public(package) fun deposit_collateral<T>(self: &mut Obligation, balance: Balance<T>) {
    let type_name = type_name::get<T>();
    obligation_collaterals::increase(&mut self.collaterals, type_name, balance::value(&balance));
    if (!balance_bag::contains<T>(&self.balances)) {
        balance_bag::init_balance<T>(&mut self.balances);
    };
    balance_bag::join(&mut self.balances, balance);
}

public(package) fun init_debt(self: &mut Obligation, market: &Market, type_name: TypeName) {
    let borrow_index = market::borrow_index(market, type_name);
    obligation_debts::init_debt(&mut self.debts, type_name, borrow_index);
}

public(package) fun increase_debt(self: &mut Obligation, type_name: TypeName, amount: u64) {
    obligation_debts::increase(&mut self.debts, type_name, amount);
}

public(package) fun decrease_debt(self: &mut Obligation, type_name: TypeName, amount: u64) {
...
public fun unlock<T: drop>(self: &mut Obligation, obligation_key: &ObligationKey, key: T) {
    assert_key_match(self, obligation_key);
...
public(package) fun set_lock<T: drop>(
    self: &mut Obligation,
    obligation_access_store: &ObligationAccessStore,
    lock_borrow: bool,
    lock_repay: bool,
    ...
    assert!(option::is_none(&self.lock_key), error::obligation_already_locked());
...
public(package) fun set_unlock<T: drop>(self: &mut Obligation, _: T) {
    assert!(
...
#[test_only]
public fun add_debt_test(self: &mut Obligation, type_name: TypeName, amount: u64) {
    obligation_debts::init_debt(&mut self.debts, type_name, sui::math::pow(10, 9));
    increase_debt(self, type_name, amount);
}

#[test_only]
public fun add_collateral_test(self: &mut Obligation, type_name: TypeName, amount: u64) {
...
#[test_only]
public fun remove_collateral_test(self: &mut Obligation, type_name: TypeName, amount: u64) {
    obligation_collaterals::decrease(&mut self.collaterals, type_name, amount);
}
