...
public struct BorrowDynamics has drop {}

public struct BorrowDynamic has copy, store {
...
public(package) fun new(ctx: &mut TxContext): WitTable<BorrowDynamics, TypeName, BorrowDynamic> {
...
public(package) fun update_borrow_index(
    self: &mut WitTable<BorrowDynamics, TypeName, BorrowDynamic>,
    type_name: TypeName,
    now: u64,
) {
    ...
    let index_delta = fixed_point32::multiply_u64(
        debt_dynamic.borrow_index,
        fixed_point32_empower::mul(time_delta, debt_dynamic.interest_rate),
    );
